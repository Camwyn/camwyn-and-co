...
5. **Version Control Exceptions:**
   ...
   * 🚨 **Agents must NEVER execute `git push`.** Pushing to remote repositories is exclusively reserved for the human developer.
...
* **Run Integrity Tests:** `lando test:integrity` or `lando ssh -s appserver -c "npm run test:integrity"`
